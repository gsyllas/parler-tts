...
- WER uses `openai/whisper-large-v3` (Greek-capable; the default
  `distil-whisper/distil-large-v2` is English-only). CLAP is English-centric, so
  treat CLAP similarity as a weak signal for Greek — rely on WER + listening to
  the wandb audio samples.
